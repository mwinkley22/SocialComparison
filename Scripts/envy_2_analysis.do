...
log using "../Log Files/summarylog_envy_2.smcl", replace

/*
Envy 2 V11:

Key variation: soliciting WTP to prevent a random participant from receiving the painting **following** a High/Low WTP participant receiving your favorite painting.
...
ttest envy_var, by(wtpHighLowCondition)
log off
...
Key variation: soliciting WTP to prevent High/Low WTP participant receiving your second favorite painting **following** them already having received your favorite. Your second favorite painting is rated highly by *both* participants
...
winsor2 destroy_var, replace trim cuts(5 95)
...
ttest destroy_var, by(wtpHighLowCondition)
ttest envy_var, by(wtpHighLowCondition)
...
ttest dp_var, by(wtpHighLowCondition)
log off
...
*Importing and analyzing Envy 2 V7
import delimited using "..\Prolific Information\EnvyExploration2V7_PIDs.csv", clear
...
import excel using "..\Raw Excel Files\Envy+2+V7_October+5,+2024_02.54.xlsx", firstrow clear
drop in 1
...
rename Destroy_2nd_highdes WTP_Prevent_High
rename Destroy_2nd_lowdes WTP_Prevent_Low
...
label variable WTP_Prevent "WTP in tasks to prevent participant from receiving for free"
...
save "..\Clean STATA Files\Envy_2_V7_clean.dta",replace
...
Key variation: soliciting WTP to prevent participant from receiving the painting **following** that High/Low WTP participant receiving your favorite painting.

Rank of Prevented Painting: Second Favorite

Within/Between: Within

Summary Statistics for Envy 2 V7:
*/
...
ttest Envy_Level, by(hypothetical)
ttest WTP_Prevent, by(hypothetical)
...
*Importing and cleaning the results for "Envy 2 V4"*
